...
class LaporanBukuBesarController extends Controller
{
    ...
    private function calculateBukuBesar(array $filters): array
    {
        $query = DB::table('detail_transaksis')
            ...
            ->leftJoin('masjids', 'transaksis.masjid_id', '=', 'masjids.id')
    ...
    private function getSaldoAwal(int $rekeningId, string $tanggalMulai, string $masjidId): float
    {
        ...
        $saldoData = DB::table('detail_transaksis')
            ...
            ->where('transaksis.tanggal', '<', $tanggalMulai)
            ->when(!empty($masjidId), fn($q) => $q->where('transaksis.masjid_id', $masjidId))
